...
// V27 — editor de pagamentos presenciais + Pix
(()=>{
  'use strict';
  const api=window.supabaseRest;
  const $=s=>document.querySelector(s);
  ...
  function closeMenu(){const m=$('#adminToolsMenu'),o=$('#adminToolsOverlay'),b=$('#adminToolsMenuBtn');m?.classList.remove('open');m?.setAttribute('aria-hidden','true');o?.classList.add('hidden');o?.setAttribute('aria-hidden','true');b?.setAttribute('aria-expanded','false');document.body.classList.remove('admin-tools-open');}
  function fill(v={}){$('#creditMachineEnabled').checked=v.credit_machine_enabled!==false;$('#pixDirectEnabled').checked=!!v.pix_direct_enabled;$('#pixKeyType').value=v.pix_key_type||'CNPJ';$('#pixKey').value=v.pix_key||'';$('#pixReceiverName').value=v.pix_receiver_name||'';$('#debitMachineEnabled').checked=v.debit_machine_enabled!==false;$('#cashEnabled').checked=v.cash_enabled!==false;}
  function collect(){const pix=$('#pixDirectEnabled').checked,key=String($('#pixKey').value||'').trim(),receiver=String($('#pixReceiverName').value||'').trim();if(pix&&!key)throw new Error('Informe a chave Pix antes de ativar o Pix.');if(pix&&!receiver)throw new Error('Informe o nome do recebedor do Pix.');return {pix_direct_enabled:pix,pix_key_type:$('#pixKeyType').value||'CNPJ',pix_key:key,pix_receiver_name:receiver,credit_machine_enabled:$('#creditMachineEnabled').checked,debit_machine_enabled:$('#debitMachineEnabled').checked,cash_enabled:$('#cashEnabled').checked};}
  ...
  $('#paymentSettingsBtn')?.addEventListener('click',open);$('#closePaymentSettings')?.addEventListener('click',()=>$('#paymentSettingsDialog')?.close());$('#cancelPaymentSettings')?.addEventListener('click',()=>$('#paymentSettingsDialog')?.close());
