...
// V24 — editor das configurações de Delivery (menu oculto)
(()=>{
  'use strict';
  const api=window.supabaseRest;
  const $=s=>document.querySelector(s);
  const DAYS=[['1','Segunda-feira'],['2','Terça-feira'],['3','Quarta-feira'],['4','Quinta-feira'],['5','Sexta-feira'],['6','Sábado'],['0','Domingo']];
  const defaultHours=()=>Object.fromEntries(DAYS.map(([d])=>[d,{enabled:true,open:'11:00',close:'22:00'}]));
  const numOrNull=v=>String(v??'').trim()===''?null:Number(v);
  const toast=(message,type='success')=>{const r=$('#toastRegion');if(!r)return;const e=document.createElement('div');e.className=`toast ${type}`;e.textContent=message;r.appendChild(e);setTimeout(()=>e.remove(),2800);};
  const showMessage=(msg,type='error')=>{const e=$('#deliverySettingsMessage');if(!e)return;e.textContent=msg;e.className=`form-message ${type}`;};
  const hideMessage=()=>$('#deliverySettingsMessage')?.classList.add('hidden');
  function closeMenu(){const m=$('#adminToolsMenu'),o=$('#adminToolsOverlay'),b=$('#adminToolsMenuBtn');m?.classList.remove('open');m?.setAttribute('aria-hidden','true');o?.classList.add('hidden');o?.setAttribute('aria-hidden','true');b?.setAttribute('aria-expanded','false');document.body.classList.remove('admin-tools-open');}
  function tierRow(t={up_to_km:'',fee:''}){const row=document.createElement('div');row.className='delivery-tier-row';row.innerHTML=`<label>Até (km)<input class="tier-km" type="number" min="0.1" max="100" step="0.1" required value="${Number.isFinite(Number(t.up_to_km))?Number(t.up_to_km):''}"></label><label>Taxa (R$)<input class="tier-fee" type="number" min="0" step="0.01" required value="${Number.isFinite(Number(t.fee))?Number(t.fee).toFixed(2):''}"></label><button class="secondary-button small delivery-tier-remove" type="button">Remover</button>`;row.querySelector('.delivery-tier-remove').onclick=()=>{row.remove();if(!$('#deliveryTiers').children.length)$('#deliveryTiers').appendChild(tierRow({up_to_km:3,fee:5}));};return row;}
  function renderHours(hours={}){const box=$('#deliveryBusinessHours');box.innerHTML='';for(const [d,label] of DAYS){const h=hours?.[d]||{enabled:true,open:'11:00',close:'22:00'};const row=document.createElement('div');row.className='delivery-hour-row';row.dataset.day=d;row.innerHTML=`<label class="delivery-hour-day"><input class="hour-enabled" type="checkbox" ${h.enabled!==false?'checked':''}><span>${label}</span></label><label>Abre<input class="hour-open" type="time" value="${h.open||'11:00'}"></label><label>Fecha<input class="hour-close" type="time" value="${h.close||'22:00'}"></label>`;box.appendChild(row);}}
  function fill(v){v=v||{};$('#deliveryEnabled').checked=v.enabled!==false;$('#deliveryEnforceHours').checked=!!v.enforce_business_hours;$('#deliveryMaxKm').value=Number(v.max_distance_km||3);$('#deliveryMinimumOrder').value=v.minimum_order_value==null?'':Number(v.minimum_order_value);$('#deliveryFreeOver').value=v.free_delivery_over==null?'':Number(v.free_delivery_over);$('#deliveryBlockedDistricts').value=Array.isArray(v.blocked_districts)?v.blocked_districts.join('\n'):'';const tiers=$('#deliveryTiers');tiers.innerHTML='';const list=Array.isArray(v.tiers)&&v.tiers.length?v.tiers:[{up_to_km:Number(v.max_distance_km||3),fee:5}];list.sort((a,b)=>Number(a.up_to_km)-Number(b.up_to_km)).forEach(t=>tiers.appendChild(tierRow(t)));renderHours(v.business_hours||defaultHours());}
  function collect(){const tiers=[...document.querySelectorAll('.delivery-tier-row')].map(r=>({up_to_km:Number(r.querySelector('.tier-km').value),fee:Number(r.querySelector('.tier-fee').value)})).filter(t=>Number.isFinite(t.up_to_km)&&t.up_to_km>0&&Number.isFinite(t.fee)&&t.fee>=0).sort((a,b)=>a.up_to_km-b.up_to_km);if(!tiers.length)throw new Error('Cadastre pelo menos uma faixa de distância.');for(let i=1;i<tiers.length;i++)if(tiers[i].up_to_km<=tiers[i-1].up_to_km)throw new Error('Os limites das faixas devem ser crescentes e sem repetição.');const max=Number($('#deliveryMaxKm').value);if(!Number.isFinite(max)||max<=0)throw new Error('Informe uma distância máxima válida.');if(tiers[tiers.length-1].up_to_km<max)throw new Error('A última faixa de taxa precisa alcançar a distância máxima.');const min=numOrNull($('#deliveryMinimumOrder').value),free=numOrNull($('#deliveryFreeOver').value);if(min!=null&&min<0)throw new Error('Pedido mínimo inválido.');if(free!=null&&free<0)throw new Error('Valor para entrega grátis inválido.');const blocked=$('#deliveryBlockedDistricts').value.split(/[\n,;]+/).map(x=>x.trim()).filter(Boolean);const hours={};document.querySelectorAll('.delivery-hour-row').forEach(r=>{hours[r.dataset.day]={enabled:r.querySelector('.hour-enabled').checked,open:r.querySelector('.hour-open').value||'00:00',close:r.querySelector('.hour-close').value||'23:59'};});return {enabled:$('#deliveryEnabled').checked,enforce_business_hours:$('#deliveryEnforceHours').checked,max_distance_km:max,minimum_order_value:min,free_delivery_over:free,blocked_districts:blocked,tiers,business_hours:hours};}
  async function open(){closeMenu();const d=$('#deliverySettingsDialog');if(!d)return;$('#deliverySettingsLoading').classList.remove('hidden');$('#deliverySettingsFields').classList.add('hidden');hideMessage();d.showModal();try{const v=await api.getDeliverySettings();fill(v);$('#deliverySettingsLoading').classList.add('hidden');$('#deliverySettingsFields').classList.remove('hidden');}catch(e){$('#deliverySettingsLoading').textContent='Não foi possível carregar: '+(e.message||e);}}
  $('#deliverySettingsBtn')?.addEventListener('click',open);$('#closeDeliverySettings')?.addEventListener('click',()=>$('#deliverySettingsDialog')?.close());$('#cancelDeliverySettings')?.addEventListener('click',()=>$('#deliverySettingsDialog')?.close());$('#addDeliveryTier')?.addEventListener('click',()=>$('#deliveryTiers')?.appendChild(tierRow()));$('#deliverySettingsForm')?.addEventListener('submit',async e=>{e.preventDefault();hideMessage();const b=$('#saveDeliverySettings');b.disabled=true;b.textContent='Salvando…';try{const body=collect();await api.saveDeliverySettings(body);toast('Configurações do Delivery salvas.');$('#deliverySettingsDialog').close();}catch(err){showMessage(err.message||'Falha ao salvar.');}finally{b.disabled=false;b.textContent='Salvar configurações';}});
})();


// V25 — editor de pagamentos
(()=>{
  'use strict';
  const api=window.supabaseRest;
  const $=s=>document.querySelector(s);
  const toast=(message,type='success')=>{const r=$('#toastRegion');if(!r)return;const e=document.createElement('div');e.className=`toast ${type}`;e.textContent=message;r.appendChild(e);setTimeout(()=>e.remove(),3000);};
  function closeMenu(){const m=$('#adminToolsMenu'),o=$('#adminToolsOverlay'),b=$('#adminToolsMenuBtn');m?.classList.remove('open');m?.setAttribute('aria-hidden','true');o?.classList.add('hidden');o?.setAttribute('aria-hidden','true');b?.setAttribute('aria-expanded','false');document.body.classList.remove('admin-tools-open');}
  function status(){const enabled=$('#infinitePayEnabled')?.checked,handle=String($('#infinitePayHandle')?.value||'').trim().replace(/^[$@]+/,'');const e=$('#infinitePayStatus');if(!e)return;e.textContent=enabled&&handle?'Configurada':enabled?'Falta InfiniteTag':'Inativa';e.className=`payment-status-badge ${enabled&&handle?'ready':enabled?'warning':''}`;}
  function fill(v={}){$('#infinitePayEnabled').checked=!!v.infinitepay_enabled;$('#infinitePayHandle').value=v.infinitepay_handle||'';$('#creditOnlineEnabled').checked=!!v.credit_online_enabled;$('#pixDirectEnabled').checked=!!v.pix_direct_enabled;$('#pixKeyType').value=v.pix_key_type||'CNPJ';$('#pixKey').value=v.pix_key||'';$('#pixReceiverName').value=v.pix_receiver_name||'';$('#debitMachineEnabled').checked=v.debit_machine_enabled!==false;$('#cashEnabled').checked=v.cash_enabled!==false;status();}
  function collect(){const handle=String($('#infinitePayHandle').value||'').trim().replace(/^[$@]+/,'');const inf=$('#infinitePayEnabled').checked,pix=$('#pixDirectEnabled').checked,credit=$('#creditOnlineEnabled').checked,key=String($('#pixKey').value||'').trim(),receiver=String($('#pixReceiverName').value||'').trim();if(inf&&!handle)throw new Error('Informe a InfiniteTag para ativar a InfinitePay.');if(pix&&!key)throw new Error('Informe a chave Pix antes de ativar o Pix direto.');if(pix&&!receiver)throw new Error('Informe o nome do recebedor do Pix.');return {infinitepay_enabled:inf,infinitepay_handle:handle,pix_direct_enabled:pix,pix_key_type:$('#pixKeyType').value||'CNPJ',pix_key:key,pix_receiver_name:receiver,credit_online_enabled:credit,debit_machine_enabled:$('#debitMachineEnabled').checked,cash_enabled:$('#cashEnabled').checked};}
  async function open(){closeMenu();const d=$('#paymentSettingsDialog');if(!d)return;$('#paymentSettingsLoading').classList.remove('hidden');$('#paymentSettingsFields').classList.add('hidden');$('#paymentSettingsMessage').classList.add('hidden');d.showModal();try{fill(await api.getPaymentSettings());$('#paymentSettingsLoading').classList.add('hidden');$('#paymentSettingsFields').classList.remove('hidden');}catch(e){$('#paymentSettingsLoading').textContent='Não foi possível carregar: '+(e.message||e);}}
  $('#paymentSettingsBtn')?.addEventListener('click',open);
  $('#closePaymentSettings')?.addEventListener('click',()=>$('#paymentSettingsDialog')?.close());
  $('#cancelPaymentSettings')?.addEventListener('click',()=>$('#paymentSettingsDialog')?.close());
  $('#infinitePayEnabled')?.addEventListener('change',status);$('#infinitePayHandle')?.addEventListener('input',status);
  $('#paymentSettingsForm')?.addEventListener('submit',async e=>{e.preventDefault();const msg=$('#paymentSettingsMessage'),b=$('#savePaymentSettings');msg.classList.add('hidden');b.disabled=true;b.textContent='Salvando…';try{const saved=await api.savePaymentSettings(collect());fill(saved);toast('Configurações de pagamento salvas.');$('#paymentSettingsDialog').close();}catch(err){msg.textContent=err.message||'Falha ao salvar.';msg.className='form-message error';}finally{b.disabled=false;b.textContent='Salvar pagamentos';}});
})();
